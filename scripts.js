const Form = {
    ...
    hidePassword() {
        ...
        eyeSVG.setAttribute('onclick', 'Form.showPassword()')
    },
    isEmpty(target) {
        let value = document.querySelector(`input#${target}`).value
        
        let isEmpty = value.length > 0 ? true : false
    ...
    buttonEnable() {
        let email = this.isEmpty('email')
        let password = this.isEmpty('password')

        if(email && password) {
            let button = document.querySelector('.button .button-submit[disabled]')
            if(button) {
